...
{
    public class MakerMulti : ModuleBase<SocketCommandContext>
    {
        public static async Task MakerMultiMenu(SocialLinkerCommand sl_command)
        {
            // If there is a cooldown session active for the command type "menu", return the method immediately.
            if (await UserCooldownMethods.IsCooldownActive(sl_command, "menu") == true)
            {
                return;
            }
            ...
            {
                await First_Use_Content_Filter_Menu.First_Use_Intro_Initialize(sl_command);
                return;
            }
            ...
            var channelSearch = Global.MenuIdList.SingleOrDefault(x => x.MenuMessage.Channel.Id == sl_command.Channel.Id);
            var userSearch = Global.MenuIdList.SingleOrDefault(x => x.User.Id == sl_command.User.Id);

            // If the channel entry exists and the user is not the same, send an error message.
            if (channelSearch != null && channelSearch.User.Id != sl_command.User.Id)
            {
                ...
                await MakerMulti_Title_Select_Menu.MakerMulti_Start(sl_command);
                return;
            }
            ...
            else if (channelSearch != null && channelSearch.User.Id == sl_command.User.Id)
            {
            ...
            else if (userSearch != null && userSearch.MenuMessage.Channel.Id != sl_command.Channel.Id)
            {
