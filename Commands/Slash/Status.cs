...
namespace SocialLinker.Commands.Slash
{
    class Status : InteractionModuleBase<SocketInteractionContext>
    {
        public async Task StatusCommandParser1(SocketSlashCommand command)
        {
        ...
        public async Task StatusCommandParser2(SocketSlashCommand command, string param)
        {
            // Get the account information of the command's user.
            var command_user_account = UserInfoClasses.GetAccount(command.User);
            ...
            var mentionedUser = (SocketUser)command.Data.Options.First().Value;

            //If the mentioned user is not null, invoke StatusScreen for the mentioned user
            if (mentionedUser != null)
            {
                await StatusScreen(command, param);
            }
            //Else, if the entered parameter is the word "detail", invoke StatusDetails for the command user
            else if (param.ToLower() == "detail")
            {
                await StatusDetails(command, param);
            }
        }

        public async Task StatusCommandParser3(SocketSlashCommand command, string param1, string param2)
        {
            // Get the account information of the command's user.
            var command_user_account = UserInfoClasses.GetAccount(command.User);
            ...
            {
                await StatusDetails(command, param2);
            }
        }

        public async Task StatusScreen(SocketSlashCommand command, [Remainder] string arg = "")
        {
            ...
            SocketUser mentionedUser = null;
            
            try
            {
                mentionedUser = (SocketUser)command.Data.Options.First().Value;
            }
            catch
            {
                // Do nothing
        ...
        public async Task StartThemeMenu(SocketSlashCommand command)
        {
        ...
        public async Task StatusDetails(SocketSlashCommand command, [Remainder] string arg = "")
        {
            // Establish variables for the command user and the command's target.
            SocketUser commandTarget = null;
            SocketUser commandUser = null;
