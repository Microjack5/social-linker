...
{
    class RenderMultiCharP2EP_PSP
    {
        QuickScenes.RenderP2EP_PSP base_p2ep_psp_rendering = new QuickScenes.RenderP2EP_PSP();
        int template_width = 480;
        int template_height = 272;

        public async Task Render_Multi_Character_Scene_P2EP_PSP(SocialLinkerCommand sl_command)
        {
            ...
            RestUserMessage loader = await channel.SendMessageAsync("", false, base_p2ep_psp_rendering.P2EP_PSP_Loading_Message().Build());
            ...
            Bitmap display_name_layer = base_p2ep_psp_rendering.Render_Name(sl_command.MakerCommand.Display_Name);
            ...
            using (Graphics graphics = Graphics.FromImage(base_template))
            {
                graphics.DrawImage(base_p2ep_psp_rendering.Bitmap_To_Color(display_name_layer, display_name_color, display_name_area), 0, 0, template_width, template_height);
                ...
                graphics.DrawImage(base_p2ep_psp_rendering.Render_Dialogue(dialogue_lines, false), 0, 0, template_width, template_height);

                Bitmap cursor = base_p2ep_psp_rendering.Render_Cursor(account);
                graphics.DrawImage(cursor, 0, 0, cursor.Width, cursor.Height);
            }

            base_template = base_p2ep_psp_rendering.Scale_Template(account, base_template);
        ...
        public Bitmap Set_Bustup_Placement(SocialLinkerCommand sl_command, UserInfoFields account, Bitmap bustup, BustupData bustup_data, int char_number)
        {
            ...
            {
                bustup = base_p2ep_psp_rendering.Invert_Bitmap(bustup);
            }
