...
{
    class RenderMultiCharP2IS_PS1
    {
        QuickScenes.RenderP2IS_PS1 base_p2is_ps1_rendering = new QuickScenes.RenderP2IS_PS1();
        int template_width = 320;
        int template_height = 240;

        public async Task Render_Multi_Character_Scene_P2IS_PS1(SocialLinkerCommand sl_command)
        {
            ...
            RestUserMessage loader = await channel.SendMessageAsync("", false, base_p2is_ps1_rendering.P2IS_PS1_Loading_Message().Build());
            ...
            using (Graphics graphics = Graphics.FromImage(base_template))
            {
                ...
                graphics.DrawImage(base_p2is_ps1_rendering.Render_Message_Window(account), 0, 0, template_width, template_height);
                ...
                graphics.DrawImage(base_p2is_ps1_rendering.Combined_Text_Layers(display_name, dialogue_lines), 0, 0, template_width, template_height);
                graphics.DrawImage(base_p2is_ps1_rendering.Render_Cursor(), 0, 0, template_width, template_height);
            }

            base_template = base_p2is_ps1_rendering.Scale_Template(account, base_template);
        ...
        public Bitmap Set_Bustup_Placement(SocialLinkerCommand sl_command, UserInfoFields account, Bitmap bustup, BustupData bustup_data, int char_number)
        {
            ...
            {
                bustup = base_p2is_ps1_rendering.Invert_Bitmap(bustup);
            }
