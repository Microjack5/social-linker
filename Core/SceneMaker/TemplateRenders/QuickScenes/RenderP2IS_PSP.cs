...
{
    internal class RenderP2IS_PSP
    {
        int template_width = 480;
        int template_height = 272;

        public async Task Render_Quick_Scene_P2IS_PSP(SocialLinkerCommand sl_command)
        {
            ...
            using (Graphics graphics = Graphics.FromImage(base_template))
            {
                ...
                else
                {
                    ...
                    maker_command_data.Character_Data_1.Base_Sprite = 1;

                    // Get the bustup data for the first sprite and render the display name to the template.
                    bustup_data = BustupDataMethods.Get_Bustup_Data(account, set_data, sl_command.MakerCommand.Character_Data_1);
                    graphics.DrawImage(Bitmap_To_Color(display_name_layer, display_name_color, display_name_area), 0, 0, template_width, template_height);
                }
        ...
        public async Task Render_System_Message(SocialLinkerCommand sl_command, MakerCommandData command_data)
        {
            ...
            {
                // Draw the input dialogue to the template.
                List<string>[] dialogue_lines = OfficialSetMethods.Line_Parser(sl_command, "P2IS-PSP", command_data.Dialogue, 4, 370);
        ...
        public Bitmap Set_Bustup_Placement(SocialLinkerCommand sl_command, UserInfoFields account, Bitmap bustup, BustupData bustup_data, OfficialSetData set_data, MakerCommandData command_data)
        {
            ...
            using (Graphics graphics = Graphics.FromImage(base_template))
            {
                ...
                if (account.P2IS_PSP_TS_Sprite_Flip == "On")
                {
                    bustup.RotateFlip(RotateFlipType.Rotate180FlipY);
                }

                switch (account.P2IS_PSP_TS_Position)
                {
                    case "Default":
                        switch (bustup_data.P2IS_PSP_Default_Position)
                        {
                            case "Left":
        ...
        public Bitmap Render_Name(string display_name)
        {
            ...
            string font_sheet = $@"{AssetDirectoryConfig.assetDirectory.assetFolderPath}//SceneMaker//Templates//P2IS-PSP//Font//p2is-psp_font_sheet.png";
        ...
        {
            Bitmap base_template = new Bitmap(template_width, template_height);
            Bitmap bright_red_square = new Bitmap(3, 3);
            Bitmap dark_red_square = new Bitmap(3, 3);

            using (Graphics graphics = Graphics.FromImage(bright_red_square))
            {
                graphics.Clear(System.Drawing.Color.FromArgb(216, 16, 16));
            }

            using (Graphics graphics = Graphics.FromImage(dark_red_square))
            {
                graphics.Clear(System.Drawing.Color.FromArgb(79, 7, 6));
            }
            ...
            using (Graphics graphics = Graphics.FromImage(base_template))
            {
                ...
                graphics.DrawImage(bright_red_square, 457, 260, 3, 3);
                graphics.DrawImage(bright_red_square, 461, 260, 3, 3);
                graphics.DrawImage(bright_red_square, 457, 264, 3, 3);
                graphics.DrawImage(bright_red_square, 461, 264, 3, 3);

                switch (cursor_version)
                {
                    case 1:
                        graphics.DrawImage(dark_red_square, 457, 260, 3, 3);
                        break;

                    case 2:
                        graphics.DrawImage(dark_red_square, 461, 260, 3, 3);
                        break;

                    case 3:
                        graphics.DrawImage(dark_red_square, 457, 264, 3, 3);
                        break;

                    case 4:
                        graphics.DrawImage(dark_red_square, 461, 264, 3, 3);
                        break;
                }
            }
        ...
        // Loading message
        public EmbedBuilder P2IS_PSP_Loading_Message()
        {
        ...
        public void Preprocess_Sprites()
        {
            //string origin_path_is = @"C:\Users\Alice\Downloads\Persona_2_Innocent_Sin-bustup\Persona_2_Innocent_Sin-bustup";
            string origin_path_ep = @"C:\Users\Alice\Downloads\Persona_2_Eternal_Punishment-bustup\Persona_2_Eternal_Punishment-bustup";
            //string output_is = @"C:\Users\Alice\Downloads\Preprocessed Output\Innocent Sin";
            string output_ep = @"C:\Users\Alice\Downloads\Preprocessed Output\Eternal Punishment";

            //int filecount = OfficialSetMethods.AttachmentCountItemDirectory(origin_path_is);
            string[] files_is = Directory.GetFiles(origin_path_ep);

            for (int i = 0; i < files_is.Length; i++)
            {
                Console.WriteLine($"Processing {files_is[i]}...");
                Bitmap base_template = new Bitmap(256, 256);
                Bitmap current_sprite = (Bitmap)System.Drawing.Image.FromFile($"{files_is[i]}");

                using (Graphics graphics = Graphics.FromImage(base_template))
                {
                    graphics.DrawImage(current_sprite, 0, 0, current_sprite.Width, current_sprite.Height);
                }

                base_template.Save($"{output_ep}//{i}.png");
            }

            Console.WriteLine("Done!");
        }

        public void Organize_Sprites()
        {
            try
            {
                string origin_path_is = @"E:\Storage\Waiting Room\Documents\Social_Linker_Final\SocialLinker\Assets\SceneMaker\Templates\P2EP-PSP\Bustup";
                string[] files_is = Directory.GetFiles(origin_path_is);

                DirectoryInfo dir = new DirectoryInfo(origin_path_is);
                DirectoryInfo[] info = dir.GetDirectories("*.*");
                int count = info.Length;

                for (int i = 0; i < count; i++)
                {
                    string[] current_dir_files = Directory.GetFiles(info[i].FullName);

                    for (int j = 0; j < current_dir_files.Length; j++)
                    {
                        Console.WriteLine($"Processing {current_dir_files[j]}...");
                        Bitmap base_template = new Bitmap(192, 192);
                        Bitmap current_sprite = (Bitmap)System.Drawing.Image.FromFile($"{current_dir_files[j]}");
                        ...
                        //Console.WriteLine($"path name is {info[i].Name.ToLower()}//{info[i].Name.ToLower()}_{j + 1}_1.png");
                        base_template.Save($"{info[i].FullName}//{info[i].Name.ToLower()}_{j + 1}_1.png");
                    }
                }
