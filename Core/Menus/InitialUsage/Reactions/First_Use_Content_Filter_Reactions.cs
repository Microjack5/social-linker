...
namespace SocialLinker.Core.Menus.InitialUsage.Reactions
{
    class First_Use_Content_Filter_Reactions
    {
        ...
        public static Task Nav_First_Use_Content_Filter_Main_Added(SocketReaction reaction, MenuIdStructure menuSession)
        {
        ...
        public static Task Nav_First_Use_Content_Filter_VC_P1_Main_Added(SocketReaction reaction, MenuIdStructure menuSession)
        {
            // Search for a content filter list that corresponds to the user's ID.
            var filterSession = Global.ContentFilterList.SingleOrDefault(x => x.User.Id == reaction.UserId);
            ...
            if (reaction.Emote.Name == "↩️")
            {
                ...
                First_Use_Content_Filter_Reactions content_nav = new First_Use_Content_Filter_Reactions();
        ...
        public static Task Nav_First_Use_Content_Filter_VC_P2IS_Main_Added(SocketReaction reaction, MenuIdStructure menuSession)
        {
            // Search for a content filter list that corresponds to the user's ID.
            var filterSession = Global.ContentFilterList.SingleOrDefault(x => x.User.Id == reaction.UserId);
        ...
        public static Task Nav_First_Use_Content_Filter_VC_P2EP_Main_Added(SocketReaction reaction, MenuIdStructure menuSession)
        {
            // Search for a content filter list that corresponds to the user's ID.
            var filterSession = Global.ContentFilterList.SingleOrDefault(x => x.User.Id == reaction.UserId);
        ...
        public static Task Nav_First_Use_Content_Filter_VC_P3_Main_Added(SocketReaction reaction, MenuIdStructure menuSession)
        {
            // Search for a content filter list that corresponds to the user's ID.
            var filterSession = Global.ContentFilterList.SingleOrDefault(x => x.User.Id == reaction.UserId);
        ...
        public static Task Nav_First_Use_Content_Filter_VC_P4_Main_Added(SocketReaction reaction, MenuIdStructure menuSession)
        {
            // Search for a content filter list that corresponds to the user's ID.
            var filterSession = Global.ContentFilterList.SingleOrDefault(x => x.User.Id == reaction.UserId);
        ...
        public static Task Nav_First_Use_Content_Filter_VC_P5_Main_Added(SocketReaction reaction, MenuIdStructure menuSession)
        {
            // Search for a content filter list that corresponds to the user's ID.
            var filterSession = Global.ContentFilterList.SingleOrDefault(x => x.User.Id == reaction.UserId);
        ...
        public static Task Nav_First_Use_Content_Filter_Main_Removed(SocketReaction reaction, MenuIdStructure menuSession)
        {
            // Search for a content filter list that corresponds to the user's ID.
            var filterSession = Global.ContentFilterList.SingleOrDefault(x => x.User.Id == reaction.UserId);
        ...
        public static Task Nav_First_Use_Content_Filter_VC_P1_Main_Removed(SocketReaction reaction, MenuIdStructure menuSession)
        {
            // Search for a content filter list that corresponds to the user's ID.
            var filterSession = Global.ContentFilterList.SingleOrDefault(x => x.User.Id == reaction.UserId);
        ...
        public static Task Nav_First_Use_Content_Filter_VC_P2IS_Main_Removed(SocketReaction reaction, MenuIdStructure menuSession)
        {
            // Search for a content filter list that corresponds to the user's ID.
            var filterSession = Global.ContentFilterList.SingleOrDefault(x => x.User.Id == reaction.UserId);
        ...
        public static Task Nav_First_Use_Content_Filter_VC_P2EP_Main_Removed(SocketReaction reaction, MenuIdStructure menuSession)
        {
            // Search for a content filter list that corresponds to the user's ID.
            var filterSession = Global.ContentFilterList.SingleOrDefault(x => x.User.Id == reaction.UserId);
        ...
        public static Task Nav_First_Use_Content_Filter_VC_P3_Main_Removed(SocketReaction reaction, MenuIdStructure menuSession)
        {
            // Search for a content filter list that corresponds to the user's ID.
            var filterSession = Global.ContentFilterList.SingleOrDefault(x => x.User.Id == reaction.UserId);
        ...
        public static Task Nav_First_Use_Content_Filter_VC_P4_Main_Removed(SocketReaction reaction, MenuIdStructure menuSession)
        {
            // Search for a content filter list that corresponds to the user's ID.
            var filterSession = Global.ContentFilterList.SingleOrDefault(x => x.User.Id == reaction.UserId);
        ...
        public static Task Nav_First_Use_Content_Filter_VC_P5_Main_Removed(SocketReaction reaction, MenuIdStructure menuSession)
        {
            // Search for a content filter list that corresponds to the user's ID.
            var filterSession = Global.ContentFilterList.SingleOrDefault(x => x.User.Id == reaction.UserId);
        ...
        public static Task VersionControlMenuDirectory(SocketReaction reaction, MenuIdStructure menuSession)
        {
            ...
            {
                // Go to a new menu.
                _ = First_Use_Content_Filter_Menu.First_Use_Content_Filter_VC_P1_Main(menuSession.User, menuSession.MenuMessage);
                return Task.CompletedTask;
            }
            ...
            {
                // Go to a new menu.
                _ = First_Use_Content_Filter_Menu.First_Use_Content_Filter_VC_P2IS_Main(menuSession.User, menuSession.MenuMessage);
                return Task.CompletedTask;
            }
            ...
            {
                // Go to a new menu.
                _ = First_Use_Content_Filter_Menu.First_Use_Content_Filter_VC_P2EP_Main(menuSession.User, menuSession.MenuMessage);
                return Task.CompletedTask;
            }
            // Persona 3
            ...
            {
                // Go to a new menu.
                _ = First_Use_Content_Filter_Menu.First_Use_Content_Filter_VC_P3_Main(menuSession.User, menuSession.MenuMessage);
                return Task.CompletedTask;
            }
            // Persona 4
            ...
            {
                // Go to a new menu.
                _ = First_Use_Content_Filter_Menu.First_Use_Content_Filter_VC_P4_Main(menuSession.User, menuSession.MenuMessage);
                return Task.CompletedTask;
            }
            // Persona 5
            ...
            {
                // Go to a new menu.
                _ = First_Use_Content_Filter_Menu.First_Use_Content_Filter_VC_P5_Main(menuSession.User, menuSession.MenuMessage);
                return Task.CompletedTask;
            }
            ...
            // Perform specific actions based on how the content filter settings are being accessed.
            if (menuSession.CurrentMenu.Contains("First_Use"))
            {
                // Activate the user's account.
                account.Account_Activated = "Yes";

                // Go to the confirmation menu.
                _ = First_Use_Content_Filter_Menu.First_Use_Content_Filter_Confirm(menuSession.User, menuSession.MenuMessage);
            }
        ...
        public void ReturnToPreviousMenu(SocketReaction reaction, MenuIdStructure menuSession)
        {
            ...
            try
            {
                ...
                Type type = Type.GetType($"SocialLinker.Core.Menus.InitialUsage.Main.First_Use_Content_Filter_Menu");
        ...
        public static List<string> CreateMenuList(SocketReaction reaction)
        {
            ...
            menu_list.Add("First_Use_Content_Filter_Main");
            ...
            {
                menu_list.Add("First_Use_Content_Filter_VC_P1_Main");
            }

            // Persona 2: Innocent Sin
            if (filterSession.P2IS_Select == true)
            {
                menu_list.Add("First_Use_Content_Filter_VC_P2IS_Main");
            }

            // Persona 2: Eternal Punishment
            if (filterSession.P2EP_Select == true)
            {
                menu_list.Add("First_Use_Content_Filter_VC_P2EP_Main");
            }

            // Persona 3
            if (filterSession.P3_Select == true)
            {
                menu_list.Add("First_Use_Content_Filter_VC_P3_Main");
            }

            // Persona 4
            if (filterSession.P4_Select == true)
            {
                menu_list.Add("First_Use_Content_Filter_VC_P4_Main");
            }

            // Persona 5
            if (filterSession.P5_Select == true)
            {
                menu_list.Add("First_Use_Content_Filter_VC_P5_Main");
            }

            // Add the last possible menu message to the string list.
            menu_list.Add("First_Use_Content_Filter_Confirm");
