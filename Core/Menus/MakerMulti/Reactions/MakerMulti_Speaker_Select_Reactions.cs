    ...
    class MakerMulti_Speaker_Select_Reactions
    {
        public static Task Nav_MakerMulti_Speaker_Select_Main(SocketReaction reaction, MenuIdStructure menuSession)
        {
            if (reaction.Emote.Name == "↩️")
            {
                ...
                else if (multimaker_session.MakerCommand.Expected_Characters == 3)
                {
                    _ = MakerMulti_Char_Select_3_Menu.MakerMulti_Character_Select_3_Main(menuSession.User, menuSession.MenuMessage);
                }
                else if (multimaker_session.MakerCommand.Expected_Characters == 4)
                {
            ...
            {
                var multimaker_session = Global.MultiMaker_Session_List.SingleOrDefault(x => x.User.Id == menuSession.User.Id);
                multimaker_session.MakerCommand.Display_Name = multimaker_session.MakerCommand.Character_Data_2.Bustup_Data.Default_Name_EN;
            ...
            {
                var multimaker_session = Global.MultiMaker_Session_List.SingleOrDefault(x => x.User.Id == menuSession.User.Id);
                multimaker_session.MakerCommand.Display_Name = multimaker_session.MakerCommand.Character_Data_3.Bustup_Data.Default_Name_EN;
            ...
            {
                var multimaker_session = Global.MultiMaker_Session_List.SingleOrDefault(x => x.User.Id == menuSession.User.Id);
                multimaker_session.MakerCommand.Display_Name = multimaker_session.MakerCommand.Character_Data_4.Bustup_Data.Default_Name_EN;
        ...
        public static Task Nav_MakerMulti_Speaker_Select_Main_Received(SocketMessage message, MenuIdStructure menuSession)
        {
            ...
            multimaker_session.MakerCommand.Display_Name = input_string;
