...
{
    class Rank_Up_Notifications_Menu
    {
        public static async Task Rank_Up_Notifications_Main(SocketGuildUser user, RestUserMessage message)
        {
            ...
            embed.WithDescription("" +
                "You can toggle notification messages on or off for whenever a social stat rank has increased.\n" +
                "\n" +
            ...
            menuSession.CurrentMenu = "Rank_Up_Notifications_Main";
        ...
        public static async Task Rank_Up_Notifications_Confirm(SocketGuildUser user, RestUserMessage message)
        {
            // Get the account information of the command's user.
            var account = UserInfoClasses.GetAccount(user);
            ...
            embed.WithDescription($"Rank up notifications have been set to **`{account.Level_Up_Notifications}`**.");
            ...
            // Edit the menu session according to the current message.
            menuSession.CurrentMenu = "Rank_Up_Notifications_Confirm";
            menuSession.MenuTimer = new Timer()
            {
                // Create a timer that expires as a "time out" duration for the user.
                Interval = MenuConfig.menu.timerDuration,
        ...
        public static EmbedBuilder MenuTimedOut(SocketGuildUser user)
        {
            ...
            embed.WithDescription($"You can edit your notifications at any time from the **`{BotConfig.bot.cmdPrefix}settings`** menu by choosing [Profile Settings] > [Rank Up Notifications].");
            return embed;
        }
    }
}
